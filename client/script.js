...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/The-More-You-Know')
    .then(res => {
  
    let repeater = document.getElementById('repeat-text');
    repeater.textContent = res.data;
    repeater.style.display = 'block';
    repeater.style.backgroundColor = 'green';
})
}
...
// function getRequest(){
//     axios.get('http://localhost:3000/query-test/?name=joe')
//     .then(response => {
...
const myQueryIs = () => {
    axios.get('http://localhost:3000/query-test/?name=the-real-slim-shady')
    .then(res => {
            console.log(res.data)
})
}

document.getElementById('query-button').addEventListener('click', myQueryIs)
